    ...
    return(
      <View style={{backgroundColor: '#fff', padding: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
        <View style={{flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center'}}>
          <Thumbnail style={{width: width*0.1, height: width*0.1}} source={require('../images/avatar.png')} />
          <View style={{marginLeft: 4}}>
            <Text style={{fontSize: FontSize(12)}}>{this.props.item.nickname || this.props.item.user_nickname}</Text>
            <Text style={{fontSize: FontSize(10)}} note>{this.props.item.signature || this.props.item.user_signature}</Text>
          </View>
        </View>
