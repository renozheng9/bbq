...
@inject(["globalStore"])
@observer
export default class ThemeDetailItem extends Component {
  constructor(props) {
    super(props);
    this.state = {
      choice: '', // 二级菜单选择的内容
  ...
  componentDidMount() {
    ...
    for(let i = 0; i < this.props.item.article_img.length; i++) {
      imgComponents.push(<Image key={this.props.item.article_img[i]} source={{uri: `${domain}image/${this.props.item.article_img[i]}-80-100.png`}} style={{margin: 5, width: imgWidth*0.3, height: imgWidth*0.3}} />)
    }
    this.setState({
  ...
  upvote = () => {
    ...
    DeviceEventEmitter.emit('updateArticle', item);
    this.setState({
      item: item
    });
    axios({
  ...
  render() {
    return (
      <View style={{padding: 10}}>
        ...
        {
          this.props.page == 'trendDetail' ?
          <View style={{marginTop: 5}}>
            <Text style={{fontSize: FontSize(12), color: '#404040'}}>{this.props.item.article_content}</Text>
            <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
              {this.state.imgComponents}
            </View>
          </View>
          :
          <TouchableWithoutFeedback onPress={() => Actions.push('trendDetail', {'trend': this.props.item})}>
            <View style={{marginTop: 5}}>
              <Text numberOfLines={8} ellipsizeMode="tail" style={{fontSize: FontSize(12), color: '#404040'}}>{this.props.item.article_content}</Text>
              <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
                {this.state.imgComponents}
              </View>
            </View>
        ...
        <View style={{flexDirection: 'row', marginTop: 5}}>
          <TouchableWithoutFeedback onPress={this.delete}>
            <View style={[styles.operation, {display: this.props.item.user_id == this.props.globalStore.userInfo.id ? 'flex' : 'none'}]}>
              <Icon name="md-trash" style={{fontSize: FontSize(14), color: '#666666', marginRight: 2}} />
              <Text style={{fontSize: FontSize(13), color: '#666666'}}>删除</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => {Actions.push('edit')}}>
            <View style={[styles.operation, {display: this.props.item.user_id == this.props.globalStore.userInfo.id ? 'flex' : 'none'}]}>
              <Icon name="md-create" style={{fontSize: FontSize(14), color: '#666666', marginRight: 2}} />
              <Text style={{fontSize: FontSize(13), color: '#666666'}}>编辑</Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={() => {this.props.page != 'trendDetail' && Actions.push('trendDetail', {'item': this.state.item})}}>
          ...
          <TouchableWithoutFeedback onPress={this.upvote}>
            <View style={styles.operation}>
              <Icon name="md-thumbs-up" style={{fontSize: FontSize(14), color: this.state.item.isUpvote ? '#53BFA2' : '#666666', marginRight: 2}} />
