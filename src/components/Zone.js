...
export default class Zone extends Component {
  ...
  componentDidMount() {
    ...
    if(this.props.userID != this.props.globalStore.userInfo.id) { // 判断是否进入本用户个人主页
      axios({ // 若进入的不是本用户个人主页,获取是否关注该用户
        url: api_user_beattention + this.props.userID,
        method: 'GET',
        headers: {
          'sign': getSign(),
          'app_type': 'android',
  ...
  render() {
    return (
      <Container>
        ...
        >
        <ImageBackground source={require('../images/theme_bg.jpg')} imageStyle={{height: width*0.35, resizeMode: 'cover'}} style={{height: width*0.55, width: width, alignItems: 'flex-start'}}>
          ...
          <View style={{flexDirection: 'row', justifyContent: 'flex-start', width: width, paddingRight: width*0.03, paddingLeft: width*0.03}}>
            <Thumbnail source={require('../images/avatar.jpg')} style={{width: width*0.16, height: width*0.16, borderRadius: width*0.08}} />
            <View style={{flex: 1, justifyContent: 'center', paddingLeft: 10}}>
              <Text style={{fontSize: FontSize(18), color: '#fff', marginBottom: 2}}>{this.props.nickname}</Text>
              <Text style={{fontSize: FontSize(15), color: '#404040'}}>{this.props.signature}</Text>
            </View>
            <View style={{position: 'absolute', right: width*0.02}}>
              <TouchableWithoutFeedback onPress={this.attend}>
                <View style={{backgroundColor: '#53BFA2', borderRadius: 4, padding: 3, alignItems: 'center', display: this.props.userID == this.props.globalStore.userInfo.id ? 'none' : 'flex'}}>
          ...
          <View style={{flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', width: width, paddingLeft: width*0.02, paddingRight: width*0.02, paddingTop: width*0.02}}>
            ...
            <TouchableWithoutFeedback onPress={() => {Actions.push('attendtheme', {'userID': this.props.userID, 'nickname': this.props.nickname})}}>
              <View style={{justifyContent: 'center', alignItems: 'center'}}>
            ...
            <TouchableWithoutFeedback onPress={() => {Actions.push('attenduser', {'userID': this.props.userID, 'nickname': this.props.nickname})}}>
              <View style={{justifyContent: 'center', alignItems: 'center'}}>
            ...
            <TouchableWithoutFeedback onPress={() => {Actions.push('fans', {'userID': this.props.userID, 'nickname': this.props.nickname})}}>
              <View style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        <Content style={{backgroundColor: '#EBEBEB', paddingTop: 10}}>
          <View style={{flex: 1, paddingTop: 10, backgroundColor: '#fff'}}>
            {/* <FlatList
              ...
              data={this.props.themeStore.themeList}
              keyExtractor={(item, index) => item.article_id.toString()}
              renderItem={({item, separators}) => 
                <ListItem item={item} type="themeDetail" />
              }
            /> */}
          </View>
        </Content>
